package com.chawran.core.mybatis.generator.service;

import java.io.FileWriter;
import java.util.ArrayList;
import java.util.List;

import com.chawran.core.mybatis.generator.dao.MyBatisDao;
...
public class MyBatisService {
	...
	public final static String model_package = "com.chawran.common.mybatis.model";
	public final static String mapper_package = "com.chawran.common.mybatis.mapper";
	public final static String dao_package = "com.chawran.common.mybatis.dao";
	
	public final static String model_path = "com\\chawran\\common\\mybatis\\model\\";
	public final static String mapper_path = "com\\chawran\\common\\mybatis\\mapper\\";
	public final static String dao_path = "com\\chawran\\common\\mybatis\\dao\\";
	...
	private String getMapperFileContent() throws Exception {
		...
		for (ColumnVO column : columnList) {
			if (j == 0) {
				if ("date".equals(column.getData_type())) {
					fileContent += "       (str_to_date(#{" + column.getColumn_name() + "},'%Y-%m-%d')\n";
				} else if ("datetime".equals(column.getData_type())) {
					fileContent += "       (str_to_date(#{" + column.getColumn_name() + "},'%Y-%m-%d %H:%i:%s')\n";
				} else {
					fileContent += "       (#{" + column.getColumn_name() + "}\n";
				}
		...
		for (ColumnVO column : columnList) {
			if (k == 0) {
				if ("date".equals(column.getData_type())) {
					fileContent += "           set " + column.getColumn_name() + " = str_to_date(#{" + column.getColumn_name() + "},'%Y-%m-%d')\n";
				} else if ("datetime".equals(column.getData_type())) {
					fileContent += "           set " + column.getColumn_name() + " = str_to_date(#{" + column.getColumn_name() + "},'%Y-%m-%d %H:%i:%s')\n";
				} else {
					fileContent += "           set " + column.getColumn_name() + " = #{" + column.getColumn_name() + "}\n";
				}
			} else {
				if ("date".equals(column.getData_type())) {
					fileContent += "              ," + column.getColumn_name() + " = str_to_date(#{" + column.getColumn_name() + "},'%Y-%m-%d')\n";
				} else if ("datetime".equals(column.getData_type())) {
					fileContent += "              ," + column.getColumn_name() + " = str_to_date(#{" + column.getColumn_name() + "},'%Y-%m-%d %H:%i:%s')\n";
				} else {
		...
		for (ColumnVO column : columnList) {
			if (l == 0) {
				if ("date".equals(column.getData_type())) {
					fileContent += "        select date_format(" + column.getColumn_name() + ",'%Y-%m-%d') " + column.getColumn_name() + "\n";
				} else if ("datetime".equals(column.getData_type())) {
					fileContent += "        select date_format(" + column.getColumn_name() + ",'%Y-%m-%d %H:%i:%s') " + column.getColumn_name() + "\n";
				} else {
					fileContent += "        select " + column.getColumn_name() + "\n";
				}
			} else {
				if ("date".equals(column.getData_type())) {
					fileContent += "              ,date_format(" + column.getColumn_name() + ",'%Y-%m-%d') " + column.getColumn_name() + "\n";
				} else if ("datetime".equals(column.getData_type())) {
					fileContent += "              ,date_format(" + column.getColumn_name() + ",'%Y-%m-%d %H:%i:%s') " + column.getColumn_name() + "\n";
				} else {
					fileContent += "              ," + column.getColumn_name() + "\n";
				}
			} 
	...
	private String getDaoFileContent() throws Exception {
		...
		fileContent += "import com.chawran.core.mybatis.dao.BaseDao;\n\n";
